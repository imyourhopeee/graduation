...
ALLOWED = {"p1", "p2", "d_near", "d_far", "inward_sign", "seat_id", "ref_w", "ref_h", "coord_norm"}


def to_engine_dict(s: dict) -> dict:
    p1 = s.get("p1", [0,0]); p2 = s.get("p2", [0,0])
    xmax = max(float(p1[0]), float(p2[0])); ymax = max(float(p1[1]), float(p2[1]))
    normalized = (xmax <= 1.01 and ymax <= 1.01)
    if normalized:
        p1_out = [float(p1[0]), float(p1[1])]
        p2_out = [float(p2[0]), float(p2[1])]
    else:
        p1_out = [int(float(p1[0])), int(float(p1[1]))]
        p2_out = [int(float(p2[0])), int(float(p2[1]))]
    return {
        "p1": p1_out, "p2": p2_out,
        ...
        # ↓ 추가: 정규화 좌표 여부/기준 렌더 크기 힌트
        "coord_norm": bool(s.get("coord_norm", normalized)),
        "ref_w": (int(s.get("ref_w")) if s.get("ref_w") else None),
        "ref_h": (int(s.get("ref_h")) if s.get("ref_h") else None),
    }
