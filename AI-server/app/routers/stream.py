...
def draw_seats(frame: np.ndarray, show_debug: bool = True, style: str = "core") -> np.ndarray:
    ...
    for s in seats:
        ...
        if isinstance(s, dict):
            ...
            seat_id = _safe_int(s.get("seat_id"), 0)
            inward  = 1 if _safe_int(s.get("inward_sign"), 1) >= 0 else -1
            ref_w   = _safe_int(s.get("ref_w"), w)
            ref_h   = _safe_int(s.get("ref_h"), h)  # 참조 해상도
        else:
            p1 = list(getattr(s, "p1")); p2 = list(getattr(s, "p2"))
            d_near = float(getattr(s, "d_near")); d_far = float(getattr(s, "d_far"))
            seat_id = _safe_int(getattr(s, "seat_id", None), 0)
            inward  = 1 if _safe_int(getattr(s, "inward_sign", None), 1) >= 0 else -1
            # SeatWire 객체에서 참조 해상도 가져오기 (없으면 현재 프레임 해상도 가정)
            ref_w   = _safe_int(getattr(s, "ref_w", None), w)
            ref_h   = _safe_int(getattr(s, "ref_h", None), h)
...
# ========= 카메라 =========
def _open_cap(src: str, width: int = 1280, height: int = 720) -> cv2.VideoCapture:
...
def stream_blur(
    ...
    h: int = Query(720, description="출력 세로 해상도"),
):
    async def _stream():
        try:
            async for chunk in iterate_in_threadpool(
