...
#--1021 추가    
    def _notify_only(self, event_type: str, seat_id: int | None, extra: dict | None = None):
        ...
        대시보드 배너를 즉시 띄우는 용도.
        """
        #  주인 세션이면 intrusion 계열 배너 브로드캐스트 자체를 막음
        if str(event_type).startswith("intrusion") and self._owner_blocks_intrusion():
            return
        ...
        try:
            ...
            ai_secret = os.getenv("AI_JWT_SECRET", "changeme")  # verifyAI에서 쓰는 키
            now = int(time.time())
            ...
            # 기본 페이로드
            payload = {
                "event_type": event_type,                      # "intrusion" 또는 "intrusion_started"
                "seat_id": int(seat_id) if seat_id is not None else None,
                "camera_id": cam_id,
                "at": now,
            ...
            # 이미 계산된 얼굴 후보를 실어주면 배너에 이름이 바로 뜸
            with self._lock:
            ...
            if idents:
                ...
                # print("[BANNER-PAYLOAD]", {"person_id": payload.get("person_id"),
                #            "identities": payload["meta"].get("identities")})
            ...
            with httpx.Client(timeout=3.0, headers=headers) as c:
                c.post(url, json=payload)  # 202 예상, 실패해도 본 로직은 계속
