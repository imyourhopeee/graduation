        ...
        try:
            ...
            if isinstance(event_data, dict):
                ...
                if ident:
                    label, conf = ident
                    with self._lock:
                        identities = list(self._last_identities)  # ✅ 다인 리스트
...
#--1021 추가    
    ...
    def process_frame(self, frame: np.ndarray, camera_id="cam2", do_blur=True, do_intrusion=True) -> InferenceResult:
        ...
        if do_intrusion:
            ...
            # [추가] 디버그 오버레이: 좌석별 coverage 하단밴드 박스 + ratio
            # SHOW_COVER_DEBUG = os.getenv("SHOW_COVER_DEBUG", "1") == "1"
            # if SHOW_COVER_DEBUG:
            #     # (선택) 사람 전체 박스도 보고 싶으면 주석 해제
            #     for (x1, y1, x2, y2) in boxes:
            #         cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 200, 255), 1)

                for i, s in enumerate(self.tripwire_app.seats):
                    r = getattr(s, "_cov_ratio", None)   # intrusion_tripwire에서 저장됨
                    b = getattr(s, "_cov_box", None)     # 하단 밴드 박스 (xyxy)
                    ...
                    if r is not None:
                        # 좌석 좌상단 근처에 ratio 표시
                        sx1 = int(min(s.p1[0], s.p2[0]))
                        sy1 = int(min(s.p1[1], s.p2[1]))
                        cv2.putText(
                            frame, f"ratio={r:.2f}",
                            (sx1, max(16, sy1 - 6)),
