...
const DDLS = [
  ...
  // `CREATE INDEX IF NOT EXISTS idx_events_meta ON events USING GIN (meta);`,
   ...
   /* ===== person_ids/confidences 배열 컬럼 추가 + 백필 + 인덱스 ===== */
   `ALTER TABLE events
      ADD COLUMN IF NOT EXISTS person_ids  JSONB DEFAULT '[]'::jsonb,
      ADD COLUMN IF NOT EXISTS confidences JSONB DEFAULT '[]'::jsonb;`,
   `UPDATE events
       SET person_ids = CASE
             WHEN person_id IS NULL     THEN '[]'::jsonb
             WHEN person_id LIKE '[%'   THEN person_id::jsonb
             ELSE to_jsonb(ARRAY[person_id])
           END,
           confidences = CASE
             WHEN confidence IS NULL    THEN '[]'::jsonb
             ELSE to_jsonb(ARRAY[confidence])
           END;`,
 
   `CREATE INDEX IF NOT EXISTS idx_events_person_ids
      ON events USING GIN (person_ids jsonb_path_ops);`
  ];
async function init() {
