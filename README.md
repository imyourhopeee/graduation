...
## 한계점
 웹캠 비활성 구간에서 발생하는 매우 짧은 촬영 행위가 누락될 수 있고, 좌석 체류 시간을 감지한 뒤 웹캠이 활성화되는 구조로 인해 얼굴 인식 단계까지 약간의 지연이 발생할 수 있음 
 또한 여러 좌석이 동시에 존재하는 실제 사무 환경에 대한 검증은 아직 충분하지 않은 상황
 향후에는 사람 형상 감지 및 얼굴 인식 모델의 성능을 강화하여 전체 감지 시간을 줄이고 미탐을 감소시키는 방향으로 개선할 필요가 있음
